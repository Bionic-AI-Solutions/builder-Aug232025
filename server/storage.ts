...
export class MemStorage implements IStorage {
  ...
  async createUser(insertUser: InsertUser): Promise<StorageUser> {
    ...
    const user: StorageUser = {
      ...
      isActive: true,
      approvalStatus: (insertUser.approvalStatus as any) || "pending",
      approvedBy: insertUser.approvedBy as any,
      approvedAt: insertUser.approvedAt as any,
      rejectionReason: insertUser.rejectionReason as any,
      lastLoginAt: null,
      createdAt: new Date(),
      updatedAt: new Date()
    } as StorageUser;
  ...
  async getMarketplaceProjects(options?: MarketplaceQueryOptions): Promise<MarketplaceQueryResult> {
    ...
    if (options?.filters) {
      ...
      if (filters.minPrice !== undefined) {
        projects = projects.filter(p => p.price >= (filters.minPrice as number));
      }
      if (filters.maxPrice !== undefined) {
        projects = projects.filter(p => p.price <= (filters.maxPrice as number));
      }
      if (filters.minRating !== undefined) {
        projects = projects.filter(p => p.rating >= (filters.minRating as number));
      }
      if (filters.tags && filters.tags.length > 0) {
        projects = projects.filter(p => p.tags.some(tag => filters.tags!.includes(tag)));
      }
    }

    if (options?.sortBy) {
      const sortOptions = options as MarketplaceQueryOptions;
      projects.sort((a, b) => {
        if (sortOptions.sortOrder === 'asc') {
          if (sortOptions.sortBy === 'price') return a.price - b.price;
          if (sortOptions.sortBy === 'rating') return a.rating - b.rating;
          if (sortOptions.sortBy === 'downloads') return a.downloadCount - b.downloadCount;
          if (sortOptions.sortBy === 'date') return a.publishedAt.getTime() - b.publishedAt.getTime();
        } else {
          if (sortOptions.sortBy === 'price') return b.price - a.price;
          if (sortOptions.sortBy === 'rating') return b.rating - a.rating;
          if (sortOptions.sortBy === 'downloads') return b.downloadCount - a.downloadCount;
          if (sortOptions.sortBy === 'date') return b.publishedAt.getTime() - a.publishedAt.getTime();
        }
        return 0;
      });
    }
