...
// Note: Express Request interface is extended globally in phase2-auth.ts
...
export function requirePersona(allowedPersonas: Persona[]) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const user = req.user as AuthUser;
    if (!allowedPersonas.includes(user.persona)) {
      return res.status(403).json({ 
        error: `Access denied: ${user.persona} persona not allowed`,
...
export function authLoggingMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  res.on('finish', () => {
    ...
    if (path.includes('/auth') && (method === 'POST' || method === 'PUT')) {
      ...
      const user = req.user as AuthUser;
      console.log(`[AUTH] ${success ? 'SUCCESS' : 'FAILED'} - ${method} ${path} - User: ${user?.email || 'anonymous'}`);
    }
  });
